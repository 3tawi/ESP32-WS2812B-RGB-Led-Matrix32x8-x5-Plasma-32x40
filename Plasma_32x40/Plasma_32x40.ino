/*
 Lien vidéo: https://youtu.be/rck-VqNmFDw
...
const uint8_t kMatrixWidth = LED_RES_X * NUM_COLS;
const uint8_t kMatrixHeight = LED_RES_Y * NUM_ROWS;
  ...
  uint16_t XY( uint8_t x, uint8_t y) {
    ...
    if( yi & 0x01) {
      if( x & 0x01) {
        ...
        i = ((LED_RES_X - x - 1) * LED_RES_Y) + reverseY;
      } else {
        i = ((LED_RES_X - x - 1) * LED_RES_Y) + y;
      }
    } else {
      if( x & 0x01) {
        uint8_t reverseY = (LED_RES_Y - 1) - y ;
...
#define FPS 8         // Maximum frames-per-second
